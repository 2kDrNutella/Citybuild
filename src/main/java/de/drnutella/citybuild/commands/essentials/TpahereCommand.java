...
//T0Do: Merge class with TpaCommand!
...
public class TpahereCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player player) {
            ...
            if (args.length == 1) {
                if (args[0].equalsIgnoreCase("accept")) {
                    if (!TPA_RUNNING_LIST.contains(player)) {
                        ...
                        if (latestTPA != null) {
                            ...
                            player.sendMessage(PREFIX + "§cDu hast keine offene Tpahere!");
                        }
                    } else {
                        player.sendMessage(PREFIX + "§cDu darfst momentan keine §btpahere §cannehmen!");
                    }
                    ...
                    if (latestTPA != null) {
                        ...
                        if (targetPlayer.isOnline()) {
                            ...
                            targetPlayer.sendMessage(PREFIX + "Der Spieler §b" + player.getName() + "§f hat deine §btpahere §cabgelehnt!");
                            player.sendMessage(PREFIX + "Du hast die §btpahere§f von §b" + targetPlayer.getName() + "§c abgelehnt");
                        }
                    } else {
                        player.sendMessage(PREFIX + "§cDu hast keine offene tpahere von diesem Spieler!");
                    }
                } else {

                    if(args[0].equalsIgnoreCase("*")){
                        if(player.hasPermission(PermissionsConfigAdapter.PERMISSION_ADMIN_TPAHERE)){
                            for (Player all : Bukkit.getOnlinePlayers()){
                                all.sendMessage("");
                                all.sendMessage(PREFIX + "§c§l" + player.getName() + " §fmöchte das du dich zu ihm teleportierst!");
                                all.spigot().sendMessage(getTPAButtonsComponent(player.getName()));
                                all.sendMessage("");
                                addRequestToList(all.getUniqueId(), new TPARequest(player, TIMEOUT_SECONDS, false));
                            }
                            return true;
                        }
                    }
                            ...
                            if (player != target) {
                                if (!hasSendTPA(player, target.getUniqueId())) {
                                    if(isPlayerIgnored(target.getUniqueId(), playerUUID)){
                                        if(player.hasPermission(PermissionsConfigAdapter.PERMISSION_ADMIN_TPAHERE)){
                                            target.sendMessage("");
                                            target.sendMessage(PREFIX + "§c§l" + player.getName() + " §fmöchte das du dich zu ihm teleportierst!");
                                        ...
                                        player.sendMessage("");
                                        player.sendMessage(PREFIX + "§fDu hast die §btpahere §ferfolgreich §fgesendet!");
                                        player.sendMessage("");

                                        target.sendMessage("");
                                        target.sendMessage(PREFIX + "§fDer Spieler §b" + player.getName() + " §fmöchte das du dich zu ihm teleportierst!");
                                        target.spigot().sendMessage(getTPAButtonsComponent(player.getName()));
                                        target.sendMessage("");
                                    ...
                                    player.sendMessage(PREFIX + "§cDu hast bereits eine tpahere an §b" + target.getName() + "§c gesendet!");
                                }
                            } else {
                                player.sendMessage(PREFIX + "§cDu kannst dir nicht selber eine tpahere schicken!");
                            }
                        } else {
                            player.sendMessage(PREFIX + "§cDer Spieler §b" + args[0] + " §cist nicht online!");
                        }
                    } else {
                        player.sendMessage(PREFIX + "§cDu darfst momentan keine §btpahere §cschicken!");
                    }
                }
                ...
                if (args[0].equalsIgnoreCase("accept")) {
                    ...
                    if (hasSendTPA(targetPlayer, playerUUID)) {
                        ...
                        if (latestTPA != null) {
                            removeRequestFromList(playerUUID, latestTPA);
                            targetPlayer.sendMessage(PREFIX + "§fDer Spieler §b" + player.getName() + "§f hat deine §btpahere §cabgelehnt!");
                            player.sendMessage(PREFIX + "§fDu hast die §btpahere§f von §b" + targetPlayer.getName() + "§c abgelehnt");
            ...
            sender.sendMessage(PREFIX + "Die Console darf keine tpahere schicken! :P");
        }
        return false;
    }
    ...
    static void runTPA(Player reciver, TPARequest tpaRequest) {
        ...
        if (sender == null) {
            reciver.sendMessage(PREFIX + "§cEtwas ist schiefgelaufen! Die tpahere wurde abgebrochen!");
            return;
        }

        if (sender.isOnline() && tpaRequest.isStillValid() && reciver.isOnline()) {
            reciver.sendMessage(PREFIX + "§fDu wirst in 5 Sekunden zu §b" + sender.getName() + "§f teleportiert");
            sender.sendMessage(PREFIX + "§fDer Spieler §b" + reciver.getName() + "§f wird in 5 Sekunden zu dir teleportiert");
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    if (sender.isOnline() && reciver.isOnline()) {
                        reciver.teleport(sender);
                        reciver.sendMessage(PREFIX + "§fDu wurdest zu §b" + sender.getName() + "§f teleportiert");
                        sender.sendMessage(PREFIX + "§fDer Spieler §b" + reciver.getName() + "§f wurde zu dir teleportiert");
                    } else {
    ...
    static TextComponent getTPAButtonsComponent(String tpaSenderName){
        return new TextComponent(
                ...
                new TextComponentUtils("§7[§a§l/tpahere accept§7]").setRunCommand("/tpahere accept " + tpaSenderName)
                        .setTexteHover("§aAkzeptiere die tpahere").toTexte(),
                new TextComponent("§f oder "),
                new TextComponentUtils("§7[§c§l/tpahere deny§7]").setRunCommand("/tpahere deny " + tpaSenderName)
                        .setTexteHover("§cLehne die tpahere ab").toTexte()
        );
    }
    ...
    static void sendSyntax(final CommandSender sender) {
        ...
        sender.sendMessage("§b/tpahere §7[§eSpielername§7] §fum eine TPA zu senden");
        sender.sendMessage("§b/tpahere accept §7[§eSpielername§7] §fum die TPA vom Spieler anzunehmen");
        sender.sendMessage("§b/tpahere accept §fum die letzte TPA anzunehmen");
        sender.sendMessage("§b/tpahere deny §7[§eSpielername§7] §fum die TPA vom Spieler abzulehnen");
        sender.sendMessage("§b/tpahere deny §fum die letzte TPA abzulehnen");
        sender.sendMessage("§b/tpahere ignore [Name] §fum einen Spieler zu ignorieren");
        sender.sendMessage("§b/tpahere ignore * §fum alle Spieler zu ignorieren");
        sender.sendMessage("");
    }
}
